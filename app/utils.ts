...
export const generateImage = async (
  ...
  const line2 = getLocalStorage(
    "vikramSamvatLine2",
    "ક્રોધીનામ - અનલ નામ સંવત્સર"
  );
...
const parseExtractedText = (text: string) => {
  ...
  const tithiMatch = lines.find(
    (line) =>
      line.includes("તિથિ") ||
      line.match(
        /ફાગણ|ચૈત્ર|વૈશાખ|જ્યેષ્ઠ|અષાઢ|શ્રાવણ|ભાદરવો|આસો|કારતક|માગશર|પોષ|મહા/
      ) ||
  ...
  if (tarikhLines.length > 0) {
    ...
    for (const line of tarikhLines) {
      ...
      const dateMatch = line.match(
        /[૦-૯0-9]{1,2}[_/\-.][૦-૯0-9]{1,2}[_/\-.][૦-૯0-9]{2,4}/
      );
      if (dateMatch) {
  ...
  const karanLine = lines.find(
    (line) =>
      ...
      (line.includes("-") &&
        line.match(
          /બવ|બાલવ|કૌલવ|તૈતિલ|ગર|વણિજ|વિષ્ટિ|શકુનિ|ચતુષ્પદ|નાગ|કિંસ્તુઘ્ન/
        ))
  );
  ...
  const suryodayLine = lines.find(
    (line) => line.includes("સૂર્યોદય") || line.match(/ઉદય/)
  );
  ...
  const suryastaLine = lines.find(
    (line) => line.includes("સૂર્યાસ્ત") || line.match(/અસ્ત/)
  );
  ...
  const rashiLine = lines.find(
    (line) =>
      line.includes("રાશી") ||
      line.match(/મેષ|વૃષભ|મિથુન|કર્ક|સિંહ|કન્યા|તુલા|વૃશ્ચિક|ધન|મકર|કુંભ|મીન/)
  );

  if (rashiLine) {
    // Try to extract the rashi name
    const rashiMatch = rashiLine.match(
  ...
  const dinMahimaHeaderIndex = lines.findIndex(
    (line) =>
      line.includes("આજ નો દિન મહિમા") || line.includes("આજનો દિન મહિમા")
  );
  ...
  Object.keys(extractedData).forEach((key) => {
    if (typeof extractedData[key] === "string") {
      ...
      extractedData[key] = extractedData[key].map((item) =>
